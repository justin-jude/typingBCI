...
from utils.gaussSmooth import gaussSmooth
...
class brainToText_decoder(object):
    ...
    def __init__(self, args):
        ...
        fh = logging.FileHandler(str(pathlib.Path(self.args['outputDir'],'training_log').resolve()))
        ...
        for i in range(len(self.args['dataset']['sessions'])):
            ...
            datasetFeatures = {
                "inputFeatures": tf.io.FixedLenSequenceFeature([768], tf.float32, allow_missing=True),
    ...
    def datasetLayerTransform(self, dat, normLayer, whiteNoiseSD, constantOffsetSD, randomWalkSD, staticGainSD, randomCut):
        ...
        sampledElectrodes = self.args['dataset']['sampledElectrodes']
        features = tf.gather(features, np.concatenate((np.array(sampledElectrodes),np.array(sampledElectrodes)+384)), axis = -1)
    ...
    def train(self):
        ...
        if early_stopping:
            self.logger.info(f"I will stop training after {self.args['nBatchesToTrain']} batches or " \
                         f"when the validation CER hasn't improved in {early_stopping_num_batches} batches.")
        else:
        ...
        for b in range(self.args['nBatchesToTrain']+1):
            ...
            if b % self.args['batchesPerVal'] == 0:
                ...
                self.logger.info(f'Val batch {b}: ' +
                        f'CER (avg): {infOut["seqErrorRate"]:.3f} ' )#+
                        # f'time: {totalSeconds:.3f}')
                ...
                # if infOut_day[len(self.args['dataset']['sessions'])-1]["per"] < best_val_per:
                    best_val_per = infOut['per']
                    infOut['best_val_per'] = best_val_per
                    # best_val_per = infOut_day[len(self.args['dataset']['sessions'])-1]["per"]
                    batches_since_improved = 0
                ...
                self.logger.info(f'Batches since validation CER improved: {batches_since_improved}')

                # early stopping
                if early_stopping & (batches_since_improved >= early_stopping_num_batches):
                    self.logger.info(f"Overall validation CER has not improved in " \
                                 f"{early_stopping_num_batches} batches. Stopping training early.")
                    break
                if early_stopping & (best_val_per <= self.args['EndCER']):
                    self.logger.info(f"Stopping at END CER ")
                    break

        trainTime_s = (datetime.now()-trainStart).total_seconds()
        trainTime_min = trainTime_s / 60

        self.logger.info(f'Best avg CER achieved: {best_val_per:.5f}')
    ...
    def inference(self, returnData=False):
        ...
        for day_val in range(0, len(self.args['dataset']['sessions'])):
            ...
            self.logger.info(f'Val batch: ' + f'CER ({self.args["dataset"]["sessions"][day_val]}): {infOut_day[day_val]["per"]:.3f}')
        ...
        infOut['seqErrorRateIndividual'] = infOut['editDistances']/infOut['trueSeqLengths']
        if returnData:
