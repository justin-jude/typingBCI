...
def formatSessionData(blocks,
    ...
    for b in blocks:
        ...
        redisFile = sorted([str(x) for x in Path(dataDir).glob('*_'+str(b)+'.mat')])
       # print(redisFile)
        redisFile = redisFile[-1]
        redisDat = scipy.io.loadmat(redisFile)
        #print(redisDat['cue'])
        #print(redisDat['startTimeStep'])
        # loop through trials
        ...
        for x in range(len(redisDat['cue'])):
            ...
            startTimeStep = redisDat['startTimeStep'][0][x]
            endTimeStep = redisDat['endTimeStep'][0][x]
            #print(redisDat['binned_neural_threshold_crossings'].shape)
            #print(startTimeStep, endTimeStep)
            ...
            trialTimes.append((endTimeStep - startTimeStep))
            ...
            newTranscription = redisDat['cue'][x][0]
    ...
    for pIter in range(len(partNames)):
        ...
        with tf.io.TFRecordWriter(str(saveDir.joinpath('chunk_0.tfrecord'))) as writer:
            ...
            for trialIdx in partIdx:
                ...
                print('Padded Transcription: ',  paddedTranscription)
                ...
                if inputFeats.shape[0] > (len(characters)*20 + 10): # this is hardcoded for the normal Stanford parameters
...
def main(args):
    ...
    idx = 5
    print('Contain Nan: ', np.any(np.isnan(sessionData['inputFeatures'][idx])))
