...
def makeTuningHeatmapT18(mapping, dat, window, num_conditions = 15):
    nFeat = dat[0].shape[2]
   # print(nFeat)
...
def heatmapPlotCirclesT18(tuning, isSig, clim, titles, layout):
    ...
    arrRows = [np.arange(64,128).astype(np.int32),
    ...
    for arrIdx in range(len(arrRows)):
        for plotIdx in range(nPlots):
            ...
            #plt.imshow(mat_sig, clim=[0,0.7],cmap='Blues')
...
def plot_tsne(trials_input, timeWindow, chanIdx, mapping):
    chanIdx = np.concatenate(( chanIdx, 384 + chanIdx), axis=-1)
    #chanIdx = chanIdx + 384
    #print(chanIdx)
    unroll_Feat = []
   ...
   # from umap import UMAP
    ...
    pca = TSNE(n_components=2, perplexity=10.0, init="pca", method="exact")
   ...
   # plt.ylabel('PC2')
    ...
    fig.savefig('T18ptsneHollow.pdf', bbox_inches='tight')
    plt.show()
    return
...
def plot_pca_trajectories(trials_input, timeWindow, chanIdx, mapping):
   ...
   # sort_pca = np.mean(combine_pca, axis =2)
   # sorted_order = np.argsort(sort_pca[0,:], axis=0)
   # print(sorted_order)
   # mapping = np.array(mapping)[sorted_order]
   # combine_pca = combine_pca[:,sorted_order,:]
    from matplotlib import cm as cm
    import matplotlib
    print(combine_pca.shape)
    plt.rcParams['axes.xmargin'] = 0
    plt.rcParams['axes.ymargin'] = 0
    #colors =  [(cm.jet(i)) for i in range(0,255)] + [('gray')]
    #colors =  [(cm.jet(np.linspace(0,1,len(mapping)-1)))] + [('gray')]
    #new_map = matplotlib.colors.LinearSegmentedColormap.from_list('new_map', colors, N=len(mapping))
    #whole_color_list = [(cm.gist_rainbow(i)) for i in range(0,256)][::-1] 
    #colors =  whole_color_list[0:130] + whole_color_list[170:258] + [('black')]
    #colors_segments =  [whole_color_list[0:20],  whole_color_list[25:40], whole_color_list[45:76], whole_color_list[80:95], #whole_color_list[105:120] ,whole_color_list[135:184],whole_color_list[193:205], whole_color_list[210:224], whole_color_list[230:245], [ 'brown', 'chocolate', 'sienna'], ('black')]
    ...
    #new_map = matplotlib.colors.LinearSegmentedColormap.from_list('new_map', colors, N=len(colors))
   # colors =  [(cm.tab10(i)) for i in range(0,5)] + [('black')]
   # colors =  [('red'),('blue'),('green'),('yellow'),('purple')]
   # new_map = matplotlib.colors.LinearSegmentedColormap.from_list('new_map', colors, N=5)
   ...
   # print(colors)
    ...
    for i in range(5):
        ...
        for kk in range(combine_pca.shape[1]):
            ...
            #color_seg = colors_segments[kk//3]
            #selection = np.linspace(0,len(color_seg)-1,3).astype(np.int32)
            #index_select = selection[int(kk%3)]
            #color = color_seg[index_select]
            #if kk == combine_pca.shape[1] -1:
             #   color = 'black'
            linestyles = ['solid'] * 31
            if i == 2:
                linestyles = ['solid', 'solid', 'dashed'] * 10
        ...
        go_cue = 250   #T17: 450 T18:250
        trial_end = 500  #T17: 700 T18:500
        iti_end = 750  #T17: 950 T18:750
        text_height = np.percentile(combine_pca[i, :, :], 99) #+ 3.5
        print(text_height)
        ...
        plt.text(((end_stim)/100) - 0.72,text_height,'Delay', fontsize=16)
        ...
        plt.text(((trial_end)/100) - 1.1,text_height,'Trial End', fontsize=16)
        ax.axvline((iti_end)/100, alpha=lines_alpha, color='gray', ls='--')
        ax.tick_params(axis="x", labelsize=16)
        ax.tick_params(axis="y", labelsize=14)
        #ax.tick_params(axis="y", labelsize=12)
        plt.text(((iti_end)/100) -0.93,text_height,'ITI End', fontsize=16)
        ...
        plt.title('Explained Variance: ' + str(round(component_variance[i]*100,2)) + '%', fontsize=24)
        plt.savefig('PC' + str(i+1) + 'T18PCATrajectoriesNewColors.pdf', bbox_inches="tight")
        plt.show()

    cumulative_variance_explained = []
    current_total = 0
    component_variance = component_variance
    for cv in component_variance:
        current_total += cv * 100
        cumulative_variance_explained.append(current_total)
    plt.plot(np.arange(len(component_variance)), cumulative_variance_explained)
    plt.title('Cumulative Variance Explained')
    plt.xlabel('Principle Components', fontsize=18)
    plt.ylabel('Cumulative Variance Explained', fontsize=18)
    plt.savefig('CumulativeVarianceExplainedT18SmoothBeforeAverage.pdf')
    plt.show()
    return
...
def plot_psth_jjj(trials_input, timeWindow, chanIdx, mapping, subtractMeansWithinBlock =False):
    ...
    for chann in chanIdx:
        ...
        for line in lgd.get_lines():
            ...
            #plt.xlim([-0.5,1.0])
        #plt.savefig('LastT18PSTHs/PSTH_movements_per_finger-Channel-' + str(chann) + '.pdf',bbox_inches='tight')
        plt.show()
